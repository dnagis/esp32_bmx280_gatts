...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        bool temp_pos = true;
        if (last_env_data->env_data.temp < 0) temp_pos = false;
        rsp.attr_value.value[0] = (int)fabs(last_env_data->env_data.temp); //fabs = valeur absolue d'un double (gestion des temps negs)
        rsp.attr_value.value[1] = ((int)(fabs(last_env_data->env_data.temp)*100)%100);
        if (temp_pos == true) rsp.attr_value.value[2] = 1; else rsp.attr_value.value[2] = 0;
        rsp.attr_value.value[3] = ((int)fabs(last_env_data->env_data.pressure-872)); //ajouter 1000-(256/2)=872 pour récupérer
        rsp.attr_value.value[4] = ((int)(fabs(last_env_data->env_data.pressure)*100)%100);
        rsp.attr_value.value[5] = (int)fabs(last_env_data->env_data.humidity);;
        rsp.attr_value.value[6] = ((int)(fabs(last_env_data->env_data.humidity)*100)%100);;
